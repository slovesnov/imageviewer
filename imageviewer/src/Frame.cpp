...
Frame::Frame(GtkApplication *application, std::string path) {
  ...
  for (auto &e : m_noImage) {
    ...
    e = gdk_pixbuf_new_from_file_at_size(getImagePath("noimage.svg").c_str(),
                                         getWidthForHeight(j), j, NULL);
  }
  ...
  m_optionsPointer = {&m_languageIndex,
                      &m_warnBeforeDelete,
                      &m_deleteOption,
                      &m_warnBeforeSave,
                      &m_showPopup,
...
#define M(id, max, variable)                                                   \
    ...
    if (j >= 0 && j < max) {                                                   \
      variable = j;                                                            \
        ...
        M(ENUM_CONFIG_TAGS::REMEMBER_THE_LAST_OPEN_DIRECTORY, 2,
          m_rememberLastOpenDirectory)
        M(ENUM_CONFIG_TAGS::SHOW_THE_TOOLBAR_IN_FULLSCREEN_MODE, 2,
          m_showToolbarFullscreen)
...
#undef M
  ...
  for (elem = formats; elem; elem = elem->next) {
    ...
    n = gdk_pixbuf_format_get_name(
        pf); // name need to store cann't use file extension
    extension_list = gdk_pixbuf_format_get_extensions(pf);
    for (i = 0; (c = extension_list[i]) != 0; i++) {
      std::string unique_marker = std::string(c) + "|" + std::string(n);

      if (seen_pairs.count(unique_marker)) {
        continue;
      }
      seen_pairs.insert(unique_marker);
  ...
  /*
   ...
   if (i) {
   printf(
   "Root: HKCR; Subkey: \".%s\"; ValueType: string; ValueName: \"\"; ValueData:
   \"{#APPNAME}\"; Flags: uninsdeletevalue; Tasks: %s\\%s%d\n", p, gn, tn, j);
   ...
   const char *A[] =
   {
   R"(Root: HKCR; Subkey: {#APPNAME}; ValueType: string; ValueName: "";
   ValueData: {#APPNAME}; Flags: uninsdeletekey; Tasks:)", R"(Root: HKCR;
   Subkey: "{#APPNAME}\DefaultIcon"; ValueType: string; ValueName: "";
   ValueData: "{app}\bin\{#APPNAME}.exe,0"; Tasks:)", R"(Root: HKCR; Subkey:
   "{#APPNAME}\shell\open\command"; ValueType: string; ValueName: ""; ValueData:
   """{app}\bin\{#APPNAME}.exe"" ""%1"""; Tasks:)" }; for (auto a : A) {
   printf("%s%s\n", a, s.c_str());
   }
   */
  ...
  gtk_widget_add_events(m_window, GDK_SCROLL_MASK | GDK_KEY_PRESS_MASK |
...
void Frame::setTitle() {
  ...
  if (noImage()) {
    ...
    if (m_mode == MODE::LIST) {
       ...
       * if LIST_ASCENDING_ORDER=1 a<b & b-a+1=m_listxy =>
      ...
      GRegex *regex =
          g_regex_new("(\\d{4})(\\d{2})(\\d{2})_?(\\d{2})(\\d{2})(\\d{2})",
      ...
      while (g_match_info_matches(match_info)) {
        ...
        if (d[0] >= 1900 && d[1] > 0 && d[1] < 13 && d[2] > 0 &&
            d[2] <= DMAX[d[1] - 1] + i && d[3] < 24 && d[4] < 60 && d[5] < 60) {
...
void Frame::addDirectory(const std::string &dir) {
  ...
  std::string s;
  // from gtk documentation order of file is arbitrary. Actually it's name (or
  // may be date) ascending order
...
void Frame::draw(cairo_t *cr, GtkWidget *widget) {
  ...
  bool windowSizeChanged =
      m_lastWidth != width ||
      m_lastHeight != height; // size of m_window is changed
  ...
  if (!m_filenameFontHeight) {
    m_filenameFontHeight =
        countFontMaxHeight("IMG_20211004_093339", filenameFontBold, cr);
  }
  ...
  if (m_mode == MODE::LIST) {
    for (k = m_listTopLeftIndex, l = 0;
      ...
      if (drawFileName) {
        ...
        //						m_filenameFontHeight*8,
        // filenameFontBold, i, j + 1,
        // m_listIconWidth, m_listIconHeight, true, 2, WHITE_COLOR,
        // true);
      }
    }
...
bool Frame::noImage() { return m_vp.empty(); }
...
void Frame::setNoImage() {
  ...
  for (int i = 0; i < TOOLBAR_INDEX_SIZE; i++) {
    ...
    setButtonState(i, oneOf(t, TOOLBAR_INDEX::OPEN, TOOLBAR_INDEX::SETTINGS));
  }
}

std::vector<FileSupported>::const_iterator
Frame::supportedIterator(std::string const &p, bool writableOnly) {
...
void Frame::switchImage(int v, bool add) {
  ...
  /* if last image in dir was scrolled down and user click next image (right
   * key) do not need to call loadImage() because it load the same image and
   * scroll it to start but nothing should happens
   */
  if (i != m_pi) {
    loadImage();
  }
...
#ifdef SHOW_THREAD_TIME
  println("t%d%s %.2lf %d", n, stopped ? " user stop" : "",
...
void Frame::mousePress(GdkEventButton *event) {
  ...
  if (event->type == GDK_DOUBLE_BUTTON_PRESS) {
    toggleFullScreen();
             ...
             GDK_BUTTON_PRESS) { // ignore double clicks GDK_DOUBLE_BUTTON_PRESS
    ...
    if (left || event->button == 2) { // left/middle mouse button
      if (m_mode == MODE::LIST) {
        if (left) { // left button check click on image
          ...
          if (cx >= m_listdx && cx < m_lastWidth - m_listdx && cy >= m_listdy &&
              cy < m_lastHeight - m_listdy &&
...
void Frame::buttonClicked(TOOLBAR_INDEX t) {
  ...
  if (i != -1) {
    if (m_mode == MODE::LIST) {
      ...
      const double k =
          t == TOOLBAR_INDEX::ZOOM_IN ? m_zoomFactor : 1 / m_zoomFactor;
  ...
  if (i != -1) {
    if (m_mode == MODE::LIST) {
      ...
      if (t == TOOLBAR_INDEX::HOME || t == TOOLBAR_INDEX::END) {
        m_pi =
            ((t == TOOLBAR_INDEX::HOME) == m_ascendingOrder) ? 0 : size() - 1;
        loadImage();
      } else {
        switchImage(t == TOOLBAR_INDEX::PAGE_UP || t == TOOLBAR_INDEX::PAGE_DOWN
                        ? sqrt(size())
                        : 1,
  ...
  if (t == TOOLBAR_INDEX::DELETE_FILE) { // m_mode==MODE::LIST can be called
                                         // from keyboard
    if (m_mode != MODE::LIST &&
        (!m_warnBeforeDelete || showDeleteDialog() == GTK_RESPONSE_YES)) {
  ...
  if (t == TOOLBAR_INDEX::SAVE_FILE) { // m_mode==MODE::LIST can be called from
                                       // keyboard
    if (m_mode != MODE::LIST &&
      ...
      if (it == m_supported.end()) { // user click save non writable file
                                     // m_warnBeforeSave=false
      ...
      bool fileInList = getPathIterator(m_modalDialogEntryText) != m_vp.end();
      bool dirChanged =
          m_dir != getFileInfo(m_modalDialogEntryText, FILEINFO::DIRECTORY);
      ...
      GError *error = NULL;
      Pixbuf pixScaled = gdk_pixbuf_scale_simple(
          m_pix, m_zoom * m_pw, m_zoom * m_ph, GDK_INTERP_BILINEAR);
      if (sameFile) {
...
void Frame::buttonClicked(int t) { buttonClicked(TOOLBAR_INDEX(t)); }
...
void Frame::showSettings() {
  ...
  for (auto e : OPTIONS) {
    ...
    if (e == LANGUAGE::LANGUAGE) {
      ...
      if (e == LANGUAGE::ONE_APPLICATION_INSTANCE) {
        ...
        gtk_container_add(
            GTK_CONTAINER(w),
            gtk_label_new(getLanguageStringC(LANGUAGE::REQUIRES_RESTARTING)));
      }
    }
  ...
  for (i = 0; i < TOOLBAR_INDEX_SIZE; i++) {
    ...
    for (l = 0; l < MAX_HOTKEYS; l++) {
      ...
      gtk_entry_set_placeholder_text(
          GTK_ENTRY(w), getLanguageStringC(LANGUAGE::CLICK_TO_SET_THE_KEY));
  ...
  for (auto e : {LANGUAGE::HOMEPAGE, LANGUAGE::SOURCE_CODE, LANGUAGE::AUTHOR,
    ...
    if (oneOf(e, LANGUAGE::HOMEPAGE, LANGUAGE::SOURCE_CODE)) {
      ...
      s = e == LANGUAGE::HOMEPAGE ? HOMEPAGE + ',' + LNG[m_languageIndex]
                                  : GITHUB;
      ...
      w = gtk_label_new(e == LANGUAGE::AUTHOR ? getLanguageStringC(e, 1)
                                              : EMAIL);
    } else {
      b = oneOf(e, LANGUAGE::WRITABLE_FORMATS, LANGUAGE::WRITABLE_EXTENSIONS);
      b1 = oneOf(e, LANGUAGE::READABLE_FORMATS, LANGUAGE::WRITABLE_FORMATS);
...
gint Frame::showDeleteDialog() {
  auto w = gtk_label_new(
      getLanguageStringC(LANGUAGE::DO_YOU_REALLY_WANT_TO_DELETE_THE_IMAGE));
...
gint Frame::showSaveDialog(bool error) {
  ...
  m_modalDialogCombo = createTextCombo(
      {getLanguageString(LANGUAGE::SAVE), getLanguageString(LANGUAGE::RENAME)},
      m_saveRename);
  ...
  gtk_entry_set_text(GTK_ENTRY(m_modalDialogEntry), m_vp[m_pi].m_path.c_str());
...
gint Frame::showModalDialog(GtkWidget *w, DIALOG o) {
  ...
  auto s = o == DIALOG::ERROR || sd
               ? getLanguageString(sd ? LANGUAGE::QUESTION : LANGUAGE::ERROR)
               : getTitleVersion();
    ...
    if (sz <= m_listxy) { // all images in screen
      ...
      if (m_ascendingOrder) {
        ...
        setNavigationButtonsState(
            m_listTopLeftIndex<max, m_listTopLeftIndex> min);
      }
    }
  } else {
    if (m_ascendingOrder) {
...
int Frame::size() { return m_vp.size(); }
...
int Frame::getFirstListIndex() { return m_ascendingOrder ? 0 : size() - 1; }
...
int Frame::getWidthForHeight(int height) { return 4 * height / 3; }
...
std::string Frame::getTitleVersion() {
  return getLanguageString(LANGUAGE::IMAGE_VIEWER) + " " +
         getLanguageString(LANGUAGE::VERSION) + " " + IMAGE_VIEWER_VERSION;
}
...
void Frame::dialogButtonClicked(LANGUAGE l) {
  ...
  if (oneOf(m_modalDialogIndex, DIALOG::DELETE, DIALOG::SAVE, DIALOG::YES_NO)) {
    if (m_modalDialogIndex == DIALOG::SAVE) {
      m_modalDialogEntryText =
          gtk_entry_get_text(GTK_ENTRY(m_modalDialogEntry));
      m_saveRename =
          gtk_combo_box_get_active(GTK_COMBO_BOX(m_modalDialogCombo));
    }
    gtk_dialog_response(GTK_DIALOG(m_modal), l == LANGUAGE::YES
                                                 ? GTK_RESPONSE_YES
                                                 : GTK_RESPONSE_NO);
    return;
  }
...
void Frame::setPopups() {
  ...
  for (auto e : m_button) {
    if (m_showPopup && gtk_widget_get_sensitive(e)) {
      gtk_widget_set_tooltip_text(e, getLanguageStringC(LANGUAGE::TOOLTIP1, i));
    } else {
...
std::string Frame::filechooser(GtkWidget *parent, const std::string &dir) {
  ...
  GtkWidget *dialog = gtk_file_chooser_dialog_new(
      ...
      onlyFolder ? GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER
                 : GTK_FILE_CHOOSER_ACTION_OPEN,
      getLanguageStringC(LANGUAGE::CANCEL), GTK_RESPONSE_CANCEL,
      getLanguageStringC(LANGUAGE::OPEN), GTK_RESPONSE_ACCEPT, NULL);
...
void Frame::addMonitor(std::string &path) {
  ...
  if (m_monitor) {
    g_signal_connect_object(G_OBJECT(m_monitor), "changed",
                            G_CALLBACK(directory_changed), NULL,
                            G_CONNECT_AFTER);
  }
}
...
void Frame::timerEventOccurred(TIMER t) {
  if (t == TIMER::DIRECTORY) {
    //		printl(timeElapse(m_lastManualOperationTime),MIN_MANULAL_OPERATION_ELAPSE,timeElapse(m_lastManualOperationTime)>=MIN_MANULAL_OPERATION_ELAPSE?"load":"not
    // load")
...
void Frame::addTimerEvent(TIMER t) {
  // if delete several files then got many delete events, the same if copy file
  // to folder so just proceed last signal
  int i = int(t);
  stopTimer(m_timer[i]);
  //	printl("stopped");
  m_timer[i] =
...
bool Frame::isOneInstanceOnly() {
  ...
  if (loadConfig(m)) {
    it = m.find(CONFIG_TAGS[int(ENUM_CONFIG_TAGS::ONE_APPLICATION_INSTANCE)]);
...
void Frame::focusIn(GtkWidget *w, int n) {
  ...
  m_modalDialogEntryOK =
      0; // need gtk_entry_set_text(GTK_ENTRY(w),"") on kill focus
  m_tmpkey[n] = INVALID_KEY; // clear hotkey
  gtk_entry_set_text(GTK_ENTRY(w), getLanguageStringC(LANGUAGE::PRESS_ANY_KEY));
}
...
void Frame::sortFiles() {
  // g_strcmp0
  // return strcoll (utf8ToLocale(a.m_path).c_str() ,
  // utf8ToLocale(b.m_path).c_str())<0;
...
VImage::iterator Frame::getPathIterator(const std::string &path) {
  return std::find_if(m_vp.begin(), m_vp.end(),
